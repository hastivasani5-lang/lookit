...
export function applyThemeToDOM(settings: ThemeSettings) {
  ...
  updateCSSVariablesForPrimaryColor(settings.primaryColor);
}

export function updateCSSVariablesForPrimaryColor(primaryColor: string) {
  if (typeof document === "undefined") {
    return;
  }
  ...
  if (rgbValues) {
    root.style.setProperty("--primary-rgb", `${rgbValues.r},${rgbValues.g},${rgbValues.b}`);
  }

  const elements = document.querySelectorAll(
    'button[class*="bg-\\[#1ec28e\\]"], .primary-color-button, [data-theme-button="primary"]',
  );
  elements.forEach((el) => {
