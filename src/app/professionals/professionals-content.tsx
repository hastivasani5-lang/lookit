      ...
      <main className="min-h-screen bg-[#eef5f3]">
        ...
        <div className="mx-auto mt-5 w-full max-w-400 px-4 md:px-8 lg:px-6 xl:px-4">
          <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-md">
            <div className="grid gap-3 md:grid-cols-[1fr_1.5fr_1fr_auto] md:items-end">
              {/* Category */}
              <label className="block">
                <span className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-gray-400">Category</span>
                <div className="relative">
                  <select
                    value={selectedCategory}
                    onChange={(event) => setSelectedCategory(event.target.value)}
                    className="w-full appearance-none rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm text-gray-700 outline-none transition focus:border-[#1ec28e] focus:bg-white focus:ring-2 focus:ring-[#1ec28e]/20"
                  >
              ...
              {/* Search */}
              <label className="block">
                <span className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-gray-400">Search Professional</span>
                <div className="flex items-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 transition focus-within:border-[#1ec28e] focus-within:bg-white focus-within:ring-2 focus-within:ring-[#1ec28e]/20">
                  <Search className="h-4 w-4 shrink-0 text-[#1ec28e]" />
                  <input
                    type="text"
                  ...
                  />
                  {professionalQuery && (
                    <button type="button" onClick={() => setProfessionalQuery("")} className="text-gray-400 hover:text-gray-600">
              ...
              <label className="block">
                ...
                <div className="flex items-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 transition focus-within:border-[#1ec28e] focus-within:bg-white focus-within:ring-2 focus-within:ring-[#1ec28e]/20">
                  ...
                  />
                  {locationText && (
                    <button type="button" onClick={() => setLocationText("")} className="text-gray-400 hover:text-gray-600">
                      <X className="h-3.5 w-3.5" />
                    </button>
                  )}
                </div>
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl px-6 py-2.5 text-sm font-bold text-white shadow-sm transition hover:scale-[1.02] hover:shadow-md active:scale-95"
        ...
        <section className="mx-auto w-full max-w-400 px-4 py-10 md:px-8 lg:px-6 xl:px-4">
          ...
          <div ref={resultsRef} className="grid gap-8 lg:grid-cols-[295px_minmax(0,1fr)] lg:items-start">
            <aside className="hidden self-start rounded-2xl border border-gray-100 bg-white shadow-md overflow-hidden lg:sticky lg:top-24 lg:z-20 lg:block lg:max-h-[calc(100vh-5rem)] lg:overflow-y-auto hide-scrollbar">
              {/* Sidebar header */}
              ...
              <div className="px-5 py-5 space-y-5">
                ...
                {/* Rating */}
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">Rating</p>
                  <div className="space-y-2">
                    {ratingOptions.map((o) => (
                      <button key={o.value} type="button" onClick={() => setSelectedRating(o.value)}
                        className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm border transition ${
                          selectedRating === o.value
                            ? "bg-[#effaf6] border-[#1ec28e] text-[#1ec28e] font-semibold"
                            : "bg-gray-50 border-gray-200 text-gray-600 hover:border-[#1ec28e]"
                        }`}>
                        <span>{o.label}</span>
                        {selectedRating === o.value && <span className="w-2 h-2 rounded-full bg-[#1ec28e]" />}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                {/* Reviews */}
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">Reviews</p>
                  <div className="relative">
                    <select value={selectedReviews} onChange={(e) => setSelectedReviews(e.target.value)}
                      className="w-full appearance-none rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm text-gray-700 outline-none transition focus:border-[#1ec28e] focus:bg-white focus:ring-2 focus:ring-[#1ec28e]/20">
                ...
                {/* Language */}
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">Language</p>
                  <div className="relative">
