...
export default function LoginPage() {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-4"
      style={{ background: "linear-gradient(135deg, #e8f5f0 0%, #d4ede6 50%, #c8e6de 100%)" }}>
          ...
          <form onSubmit={handleLogin} className="w-full">
            ...
            <div className="flex gap-2 mb-5 p-1 bg-gray-100 rounded-xl">
              {(["student","professional"] as UserRole[]).map((r) => (
                <button key={r} type="button" onClick={() => setLoginRole(r)}
                  className={`flex-1 py-2 rounded-lg text-xs font-semibold capitalize transition-all ${loginRole === r ? "bg-white text-[#0d7a57] shadow" : "text-gray-500"}`}>
            ...
            <div className="flex justify-center gap-3">
              <button type="button" onClick={handleGoogleLogin} disabled={googleLoading}
                className="flex items-center gap-2 px-4 py-2 border border-gray-200 rounded-lg text-xs text-gray-600 hover:bg-gray-50 transition">
            ...
            <div className="flex gap-2 mb-4 p-1 bg-gray-100 rounded-xl">
              {(["student","professional"] as UserRole[]).map((r) => (
                <button key={r} type="button" onClick={() => setRegRole(r)}
                  className={`flex-1 py-2 rounded-lg text-xs font-semibold capitalize transition-all ${regRole === r ? "bg-white text-[#0d7a57] shadow" : "text-gray-500"}`}>
                  {r === "student" ? "🎓 Student" : "💼 Professional"}
                </button>
              ))}
            </div>
