...
export async function GET() {
  try {
    ...
    const reviews = allReviews
      .filter((r) => user.role === "professional" ? r.professionalId === user.id : r.studentId === user.id)
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    return NextResponse.json({ reviews, totalReviews: reviews.length, averageRating: formatAverageRating(reviews), lastUpdated: new Date().toISOString() });
  } catch {
    return NextResponse.json({ reviews: [] });
  }
}
...
export async function POST(request: Request) {
  const session = await getServerSession(authOptions);
  if (!session?.user?.id || session.user.role !== "student") return NextResponse.json({ message: "Unauthorized." }, { status: 401 });
  const student = await getUserById(session.user.id);
  if (!student || student.role !== "student") return NextResponse.json({ message: "Unauthorized." }, { status: 401 });
  ...
  if (!professional || professional.role !== "professional" || professional.approvalStatus !== "approved") return NextResponse.json({ message: "Professional not found." }, { status: 404 });
  const record = await addReview({ id: `review-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, studentId: student.id, studentName: student.name, studentEmail: student.email, professionalId: professional.id, professionalName: professional.name, contentId, contentType, rating, review, createdAt: new Date().toISOString() });
